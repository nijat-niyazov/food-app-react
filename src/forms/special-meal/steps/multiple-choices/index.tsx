...
const MultipleChoices = ({ fieldName, options: categories, register, handleChange, ...rest }: MultipleChoicesProps) => {
  return (
    categories && (
      <div className="grid gap-10 md:gap-14 mt-10 ">
        {categories.map((category, i) => (
          <motion.div
            initial={{ opacity: 0, x: 100 }}
            whileInView={{ opacity: 1, x: 0, transition: { duration: 0.5, delay: i * 0.1 } }}
            key={i}
            className="relative  border-1 border-black/30 pt-8 p-5 md:pt-5 rounded-md"
          >
            <CategoryTitle img={category.imgTitle} title={category.title} />

            <ul className="grid gap-y-2 md:grid-cols-3 md:gap-x-20">
              {category.options.map((values) => {
                ...
                return (
                  <motion.li
                    whileHover={{ scale: 1.2, originX: 0, transition: { duration: 0.3 } }}
                    key={id}
                  ...
                  >
                    ...
                    <label htmlFor={id} className="cursor-pointer flex items-center gap-3 grow">
                      {name}
                      <span className={cn("text-lg font-medium italic ml-auto")}>{price}$</span>
                    </label>
                  </motion.li>
                );
              })}
