...
function Skeletons() {
  return Array.from({ length: 3 }).map((_, i) => (
    <li key={i} className="flex items-center gap-3 bg-oranged p-2 ">
...
function Meals() {
  return Array.from({ length: 3 }).map((_, i) => (
    <li key={i}>
      <Link to={"/"} className="flex items-center gap-3 bg-oranged p-2 ">
        <img src={fast_food} className="w-14 object-cover h-14 rounded-full" />
        <div className="grow overflow-hidden">
          <p className="mb-2 text-darkblue font-semibold">Farm House Xtreme Pizza</p>
