...
function BasketMenuItem({ item }: { item: BasketItemType }) {
  return (
    <AnimatePresence>
      ...
      >
        <span className="rounded-full text-2xl font-bold bg-oranged text-white  w-11 h-11 grid place-content-center">{item.quantity}x</span>

        <span className="text-greeny text-xl font-semibold">$ {item.price}</span>

        <ul className="grid gap-1.5 grow">
          <li className="text-darkblue text-base font-semibold">{item.title}</li>
        ...
        {item.size && (
          <p className="col-span-2 w-full flex items-center justify-between">
            Option: <span className="bg-darkblue text-white rounded-s px-2 py-1">{item.size}</span>
          </p>
        )}
      </MotionDiv>
    </AnimatePresence>
